...
func (s *WinFS) Stat(name string) (fs.FileInfo, error) {
	out, err := s.ExecOutput(statCmdTemplate, ps.DoubleQuotePath(name), exec.PS())
	if err != nil {
...
func (s *WinFS) Sha256(name string) (string, error) {
	sum, err := s.ExecOutput("(Get-FileHash %s -Algorithm SHA256).Hash.ToLower()", ps.DoubleQuotePath(name), exec.PS())
	if err != nil {
...
func (s *WinFS) Remove(name string) error {
	...
	if err := s.Exec("cmd.exe /c del %s", ps.DoubleQuotePath(name)); err != nil {
...
func (s *WinFS) removeDir(name string) error {
	if err := s.Exec("cmd.exe /c rmdir /q %s", ps.DoubleQuotePath(name)); err != nil {
...
func (s *WinFS) removeDirAll(name string) error {
	if err := s.Exec("cmd.exe /c rmdir /s /q %s", ps.DoubleQuotePath(name)); err != nil {
		return fmt.Errorf("rmdir %s: %w", name, err)
	}
...
func (s *WinFS) MkdirAll(name string, _ fs.FileMode) error {
	if err := s.Exec("New-Item -ItemType Directory -Force -Path %s", ps.DoubleQuotePath(name), exec.PS()); err != nil {
...
func (s *WinFS) Mkdir(name string, _ fs.FileMode) error {
	if err := s.Exec("cmd.exe /c mkdir %s", ps.DoubleQuotePath(name)); err != nil {
...
func (s *WinFS) Touch(name string) error {
	if err := s.Exec("Get-Item %[1]s -ErrorAction SilentlyContinue | Set-ItemProperty -Name LastWriteTime -Value (Get-Date); if (!$?) { New-Item %[1]s -ItemType File }", name, exec.PS()); err != nil {
...
func (s *WinFS) Chtimes(name string, atime, mtime int64) error {
	...
	if err := s.Exec("$file = Get-Item %s; $file.LastWriteTime = %s; $file.LastAccessTime = %s", ps.DoubleQuotePath(name), ps.SingleQuote(mtimeMs), ps.SingleQuote(atimeMs), exec.PS()); err != nil {
...
func (s *WinFS) Chmod(name string, mode fs.FileMode) error {
	...
	if err := s.Exec("attrib %sR %s", attribSign, ps.DoubleQuotePath(name)); err != nil {
...
func (s *WinFS) Truncate(name string, size int64) error {
	if err := s.Exec("Set-Content -Path %s -Value $null -Encoding Byte -Force -NoNewline -Stream '::$DATA' -Offset %d", ps.DoubleQuotePath(name), size, exec.PS()); err != nil {
...
func (s *WinFS) Getenv(key string) string {
	out, err := s.ExecOutput("[System.Environment]::GetEnvironmentVariable(%s)", ps.SingleQuote(key), exec.PS(), exec.TrimOutput(true))
	if err != nil {
		return ""
	}
	return out
...
func (s *WinFS) Rename(oldpath, newpath string) error {
	if err := s.Exec("Move-Item -Path %s -Destination %s", ps.DoubleQuotePath(oldpath), ps.DoubleQuotePath(newpath), exec.PS()); err != nil {
